...
`02-data-model-and-types.md`) controls:
...
3. **Particle preset** — a key referencing a preset configuration in the
   particle manager (see `13-particles.md`). Examples: floating leaves for a
...
4. **Ambient sound key** — a key referencing a registered loop in the audio
   manager (see `12-audio.md`). Examples: forest birds, cave echoes, ocean
...
- `src/environment/environmentManager.ts` — applies and tears down an
  ...
  changes; coordinates between CSS, particles, and audio
