  ...
  >
    ...
    <template v-if="isSpeedTest">
      <div class="lc-content">
        ...
        <div v-if="speedTestHistory && speedTestHistory.length > 0" class="lc-history">
        ...
        <div class="lc-actions">
          <button class="lc-btn lc-btn-primary" @click="emit('next')">Next Level →</button>
          <button class="lc-btn lc-btn-secondary" @click="emit('retry')">Retry ↺</button>
          <button class="lc-btn lc-btn-secondary" @click="emit('levelSelect')">Level Select</button>
        </div>
      </div>
    </template>
    ...
    <template v-else>
      <div class="lc-content">
        <div class="lc-header">
          <div :class="`lc-result-badge ${stats.passed ? 'badge-pass' : 'badge-fail'}`">
            {{ stats.passed ? "✓ Level Complete!" : "✗ Not quite!" }}
          </div>
          <h2 class="lc-level-title">{{ isSpeedCheckLevel ? `Level ${levelNumber} — Speed Check` : `Level ${levelNumber}` }}</h2>
        </div>
        ...
        <div class="lc-stats">
          <div class="lc-stat-row">
            ...
            <div class="lc-stat-bar-track">
              <div
                class="lc-stat-bar"
                :class="stats.wpm >= thresholds.wpm ? 'stat-bar-pass' : 'stat-bar-fail'"
                :style="`width: ${wpmBarWidth}%`"
                ref="wpmBar"
              ></div>
              <div class="lc-threshold-marker" :style="`left: ${wpmThresholdLeft}%`"></div>
            </div>
            <span
              class="lc-stat-val"
          ...
          <div class="lc-stat-row">
            ...
            <div class="lc-stat-bar-track">
              <div
                class="lc-stat-bar"
                :class="stats.accuracy >= thresholds.accuracy ? 'stat-bar-pass' : 'stat-bar-fail'"
            ...
            <span
              class="lc-stat-val"
              :class="stats.accuracy >= thresholds.accuracy ? 'val-pass' : 'val-fail'"
            >
              {{ stats.accuracy }}%
        ...
        <div :class="`lc-message ${stats.passed ? 'lc-message-pass' : 'lc-message-fail'}`">
          {{ feedbackMessage }}
        </div>
        ...
        <div class="lc-actions">
          <button v-if="stats.passed" class="lc-btn lc-btn-primary" @click="emit('next')">
            {{ levelNumber >= MAX_LEVEL ? "🏆 See Finale" : "Next Level →" }}
          </button>
          <button v-else class="lc-btn lc-btn-primary" @click="emit('retry')">Try Again ↺</button>
          <button v-if="!stats.passed" class="lc-btn lc-btn-secondary" @click="emit('next')">
            Skip ahead anyway →
          </button>
          <button class="lc-btn lc-btn-secondary" @click="emit('levelSelect')">Level Select</button>
        </div>
...
const isSpeedCheckLevel = computed(() => levelDef.value.isSpeedTest || props.levelNumber === MAX_LEVEL);
const thresholds = computed(() => DIFFICULTY_THRESHOLDS[props.difficulty]);

const wpmBarWidth = computed(() =>
  Math.min(100, Math.round((props.stats.wpm / Math.max(thresholds.value.wpm, 1)) * 100)),
);
const wpmThresholdLeft = computed(() =>
  Math.min(100, Math.round((thresholds.value.wpm / Math.max(props.stats.wpm, thresholds.value.wpm, 1)) * 100)),
);
...
const retryRec = computed((): string | null => {
  ...
  const chars = levelDef.value.availableLetters.split("").filter((c) => c !== " ");
  let maxErrors = -1;
  let worstErrorChar = "";
  for (const char of CURRICULUM_ORDER) {
    if (!chars.includes(char)) continue;
    const count = props.stats.charErrors[char] ?? 0;
    if (count >= maxErrors) { maxErrors = count; worstErrorChar = char; }
  }
  let maxAvgTime = -1;
  let slowestChar = "";
  for (const char of CURRICULUM_ORDER) {
    ...
    if (avg >= maxAvgTime) { maxAvgTime = avg; slowestChar = char; }
  }
  const lines: string[] = [];
  ...
  if (hasErrors && hasTiming && worstErrorChar === slowestChar) {
    ...
    lines.push(`<strong>"${worstErrorChar}"</strong> was your most-missed and slowest key.${hint}`);
  } else {
    if (hasErrors) {
      const level = CHAR_TO_LEARN_LEVEL[worstErrorChar];
      const hint = level ? ` — replay Level ${level} to drill it` : "";
      lines.push(`You missed <strong>"${worstErrorChar}"</strong> most often${hint}.`);
    }
    if (hasTiming) {
      const level = CHAR_TO_LEARN_LEVEL[slowestChar];
      const hint = level ? ` — replay Level ${level} to build speed` : "";
      lines.push(`<strong>"${slowestChar}"</strong> was your slowest key${hint}.`);
    }
  }
...
const feedbackMessage = computed(() => {
  ...
  if (team === "pokemon") {
    if (stats.passed) {
      ...
      if (stats.accuracy < DIFFICULTY_THRESHOLDS["easy"].accuracy) return "⚡ Slow down and aim carefully — Pikachu's strikes need to hit their mark!";
    ...
    if (stats.passed) {
      ...
      if (stats.accuracy < DIFFICULTY_THRESHOLDS["easy"].accuracy) return "🌈 Easy does it! Friendship takes patience — slow down and type with care!";
