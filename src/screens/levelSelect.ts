...
const CS_TRIGGER_LEVEL: Record<number, number> = { 1: 2, 2: 5, 3: 8, 4: 11, 5: 14 };
...
export function renderLevelSelect(
  ...
  const arcMap: Record<number, LevelDefinition[]> = { 1: [], 2: [], 3: [], 4: [], 5: [] };
  for (const lvl of LEVELS) arcMap[lvl.arc].push(lvl);

  const totalCompleted = Object.values(profile.levelRecords).filter(r => r.completed).length;
  ...
  screen.addEventListener('keydown', e => {
    ...
    const el = (e.target as HTMLElement).closest<HTMLElement>('[data-level], [data-cutscene]');
    if (el) { e.preventDefault(); el.click(); }
  });

  // Staggered entrance — after animation, lock opacity so shake can't steal it
  requestAnimationFrame(() => {
    const items = screen.querySelectorAll<HTMLElement>('.ls-level-card, .ls-cs-node');
    items.forEach((el, i) => {
...
function buildArc(
  ...
  const levelCards = levels.map(lvl => buildLevelCard(lvl, profile, attempted)).join('');
...
function buildLevelCard(
  ...
  const stateClass = completed ? 'ls-card-done'
    : unlocked     ? `ls-card-open${isNext ? ' ls-card-next' : ''}`
                   : 'ls-card-locked';

  const newLetters   = lvl.isSpeedTest ? '' : computeNewLetters(lvl);
  const letterChips  = newLetters.split('')
    .map(c => `<span class="ls-chip">${c.toUpperCase()}</span>`)
    .join('');

  const statsRow = completed && record
  ...
  const speedBadge = lvl.isSpeedTest
    ? `<span class="ls-speed-badge">⚡ Speed</span>`
    : '';
...
function computeNewLetters(lvl: LevelDefinition): string {
  ...
  return lvl.availableLetters.split('').filter(c => !prevSet.has(c)).join('');
}
