...
interface KeyMeta { capId: string; labelId: string; finger: string }
...
export function renderFingerGuide(
  ...
  const prevLetters = levelNumber > 1 ? (LEVEL_LETTERS[levelNumber - 1] ?? '') : '';
  ...
  if (levelDef.isSpeedTest) {
    ...
    const newKeys = [...newSet].map(k => `<kbd>${k.toUpperCase()}</kbd>`).join(' ');
  ...
  // ── Hand SVGs ────────────────────────────────────────────────────────────────
  const leftHandSvgWrap  = screen.querySelector('.fg-hand-left  .fg-hand-svg') as HTMLElement;
  const rightHandSvgWrap = screen.querySelector('.fg-hand-right .fg-hand-svg') as HTMLElement;
  ...
  legend.innerHTML = FINGER_ORDER
    .filter(f => usedFingers.has(f))
    .map(f => `
